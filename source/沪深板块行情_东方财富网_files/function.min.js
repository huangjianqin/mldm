    ...
    ns[name] = new function() {
        ...
        */
        ...
        _this.queryTop = function(form) {
            form.action = "list.html#" + form.sortField.value + "?sortType=" + form.sortType.value + "&sortRule=-1";
            form.target = "_blank";
            form.submit();
        };
...
//美股行情 CSS
function USStockCssClass(dataRow) {
    this.price = quote.getCssByPreclose(dataRow[5], dataRow[4]); //现价;
    this.change = quote.getCssByChange(dataRow[6]); //涨跌;
    this.open = quote.getCssByPreclose(dataRow[3], dataRow[4]); //今开
    this.high = quote.getCssByPreclose(dataRow[9], dataRow[4]); //最高
    this.low = quote.getCssByPreclose(dataRow[10], dataRow[4]); //最低
}
...
function AHCssClass(dataRow){
	this.h = quote.getCssByChange(parseFloat(dataRow[4]));
	this.a = quote.getCssByChange(parseFloat(dataRow[9]));
}
...
(function(ns) {
    ...
    function NewHSModel(query, view) {
        Model.call(this, query, view);
        this.url = config.AHQuoteList;
    }

    NewHSModel.prototype = Model.extend();

    //override
    NewHSModel.prototype.update = function () {
